...
Aria streams the LLM token-by-token anyway, so she can synthesize a short
...
CASES = [
    ("one long sentence", ["Aria on netreach is blocked on a storage decision, Postgres or SQLite?"]),
    ("split at the comma", ["Aria on netreach is blocked on a storage decision.", "Postgres or SQLite?"]),
    ("short opener first", ["Heads up.", "Aria on netreach is blocked on a storage decision.", "Postgres or SQLite?"]),
]
